...
if [[ -d "{image_path}" ]]; then
  # Newer version of yq changed the default output to json and prints an annoying warning if --output-format is not provided.
  # -r means raw output without the quotes.
  readonly DIGEST=$("$yq" eval --output-format json -r '.manifests[0].digest | sub(":"; "-")' "{image_path}/index.json")
